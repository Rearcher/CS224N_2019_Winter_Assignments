...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.source(source_padded.permute(1, 0)) # (b, src_len, e)
        X = X.permute(1, 0, 2) # (src_len, b, e)
        ...
        init_decoder_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1) # (b, 2 * h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings.target(target_padded.permute(1, 0)) # (b, tgt_len, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(torch.unsqueeze(dec_hidden, dim=-2), enc_hiddens_proj.permute(0, 2, 1))
